...
public class PokerTableController {
	...
	ArrayList<GamePlayPlayerHand> players = new ArrayList<GamePlayPlayerHand>();
	...
	private void handleDraw()
	{
		...
		for (Player p: mainApp.GetPlayers())
		{
				...
				else if (p.getiPlayerPosition() == 2)				
				{
					...
					ImageView img = new ImageView(new Image(getClass().getResourceAsStream(
							strCard), 75, 75, true, true));
					ImageView i = (ImageView) h2P2.getChildren().get(iCardDrawn - 1);
					ImageView iCardFaceDown = (ImageView) HboxCommonArea.getChildren().get(0);
					final ParallelTransition transitionForward = createTransition(i, iCardFaceDown,
							new Image(getClass().getResourceAsStream(strCard), 75, 75, true, true));
				...
				else if (p.getiPlayerPosition() == 4)				
				{
					...
					ImageView img = new ImageView(new Image(getClass().getResourceAsStream(
							strCard), 75, 75, true, true));
					ImageView i = (ImageView) h4P4.getChildren().get(iCardDrawn - 1);
					ImageView iCardFaceDown = (ImageView) HboxCommonArea.getChildren().get(0);
					final ParallelTransition transitionForward = createTransition(i, iCardFaceDown,
							new Image(getClass().getResourceAsStream(strCard), 75, 75, true, true));
		...
		if (iCardDrawn == 5)
		{
			...
			System.out.println("Winner with a " +hands.get(0).getHandStrength());
				}
			}
	...
	private void handlePlay() {
		...
		h2P2.getChildren().clear();
		h3P3.getChildren().clear();
		h4P4.getChildren().clear();
